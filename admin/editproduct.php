<?php
	...
	if(isset($_POST) & !empty($_POST)){
		...
		if(isset($_FILES) & !empty($_FILES)){
			...
			if(isset($name) && !empty($name)){
				if(($extension == "jpg" || $extension == "jpeg") && $type == "image/jpeg" && $size<=$max_size){
					...
					$fmsg = " Seuls les formats jpg et png sont acceptés";
				}
			}else{
				$fmsg = "Sélectionnez un fichier";
			}
		}else{
		...
		if($res){
			$smsg = "Produit mis à jour";
		}else{
			$fmsg = "Téléversement non réussi";
		}
	}
?>
		...
		<?php if(isset($smsg)){ ?><div class="alert alert-success" role="alert"> <?php echo $smsg; ?> </div><?php } ?>
			...
			<form method="post" enctype="multipart/form-data">
			  ...
			  <input type="hidden" name="filepath" value="<?php echo $r['thumb']; ?>">
			    <label for="Productname">Nom du product</label>
			  ...
			  <div class="form-group">
			    ...
			    <br>
			    	...
			    	<a href="delprodimg.php?id=<?php echo $r['id']; ?>">Supprimer</a>
			    <?php }else{ ?>
			    <input type="file" name="productimage" id="productimage">
			    <p class="help-block">Seuls les formats jpg et png sont acceptés</p>
			    <?php } ?>
			  </div>
